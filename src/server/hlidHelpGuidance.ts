...
export const TOPIC_GUIDANCE: Record<HlidHelpTopic, string[]> = {
	...
	orchestration: [
		...
		"Configured workspaces may use shared, when-available, or required managed Git worktrees for durable Hlid children. A per-delegation workspace_mode can select shared or explicitly acknowledge a worktree from exact HEAD. Hlid creates no checkpoints, never includes dirty source changes in the worktree, resumes the exact recorded path, and retains terminal worktrees until cleanup_hlid_worktree confirms they are clean and have no unique commits. Provider-native subagents are unchanged; Routines remain shared.",
	...
	project_preview: [
		"Start one session-scoped web server as a Project Preview, or open an arbitrary public HTTP or HTTPS URL as a Browser target. They share Hlid's managed capture, semantic refs, bounded control, and presentation surface.",
		"Browser targets reuse project_preview.use_real_browser_profile. Private and loopback URLs require explicit allow_private_network and remain restricted to the exact approved origin. Hlid blocks downloads and file URLs and does not expose clipboard, raw CDP, or device-permission controls.",
		"Optional Browser interaction recording stores only the exact screenshot states produced by Browser captures and controls plus sanitized action metadata. stop_web_browser_recording publishes a bounded self-contained replay Relic. Typed text is not copied into timeline metadata, but rendered screenshots can contain visible form values.",
		"Use restart_project_preview to restart an existing session Preview with its recorded command and trust boundary.",
		"Preview tools are bounded to the active workspace, session, and preview-local routes. Browser tools are session-scoped and public-web capable.",
	],
	mcp: [

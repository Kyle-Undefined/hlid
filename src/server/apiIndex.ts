...
export const API_ENDPOINTS: ApiEndpoint[] = [
	...
	},
	{
		...
		server: "api",
		desc: "Register or refresh one validated browser PushSubscription, with optional named-device request, problem, completion, privacy, runtime-threshold, timed-pause, and manual-pause preferences.",
	},
	{
		method: "PATCH",
		path: "/api/push/subscriptions",
		server: "api",
		desc: "Update one subscribed device's request, problem, completion, generic/detailed privacy, runtime-threshold, timed-pause, or manual-pause preferences.",
	},
	{
		method: "POST",
		path: "/api/push/devices",
		server: "api",
		desc: "List non-secret named Web Push device health for this installation. An optional endpoint marks the current device without returning any endpoint or encryption key.",
	},
	{
		method: "PATCH",
		path: "/api/push/devices",
		server: "api",
		desc: "Rename one subscribed device by its opaque installation-local id.",
	},
	{
		method: "DELETE",
		path: "/api/push/devices",
		server: "api",
		desc: "Revoke one subscribed device by opaque id and permanently remove its endpoint and Web Push encryption material.",
	},
	{
		method: "POST",
		path: "/api/push/test",
		server: "api",
		desc: "Send a real encrypted notification preview only to the current owned endpoint. Supports basic delivery, approval, question, plan review, problem, single-completion, and completion-batch forms using the device's Generic or Detailed preference, without changing session state or claiming device display.",
	},
	{
		method: "DELETE",
		path: "/api/push/subscriptions",
	...
	},
	{
		...
		server: "api",
		desc: "Read the installation-wide Default, Always notify, Notify once, or Mute override for one exact Hlid session.",
	},
	{
		method: "PATCH",
		path: "/api/push/session-overrides",
		server: "api",
		desc: "Set one session to Default, persistent Notify, one accepted Notify once, or Mute. Default removes the durable override.",
	},
	{
		method: "GET",
	...
	},
	{
		...
		server: "api",
		desc: "Inspect an enabled ACP agent's live model catalog in an optional exact configured workspace, with Hlid integrations applied but without Hlid's OpenCode model-visibility filter. Used on demand by Forge so integrated and filtered-out models remain editable.",
	},
	{
		method: "POST",
	...
	},
	{
		...
		path: "/ollama",
		server: "api",
		desc: "Windows Ollama availability, vendor-installer setup progress, local-only model inventory, actual loaded context, pull state, exact WSL network readiness, and the narrow Hlid Hyper-V firewall rule state.",
	},
	{
		method: "POST",
		path: "/ollama/setup/download",
		server: "api",
		desc: "Begin one background download of the latest stable official Ollama Windows installer, or resume Authenticode verification of a retained installer whose GitHub release digest already matched. Hlid uses private staging and never bundles or silently installs the vendor runtime.",
	},
	{
		method: "GET",
		path: "/ollama/setup",
		server: "api",
		desc: "Lightweight Windows Ollama availability and installer-setup progress for polling without repeating model inventory, firewall, or WSL network discovery.",
	},
	{
		method: "DELETE",
		path: "/ollama/setup/download",
		server: "api",
		desc: "Cancel Hlid's active Ollama Windows installer download and remove its partial staged artifact.",
	},
	{
		method: "POST",
		path: "/ollama/setup/launch",
		server: "api",
		desc: "Reverify and open Hlid's staged official OllamaSetup.exe through the interactive Windows shell. The vendor installer retains installation and update ownership.",
	},
	{
		method: "POST",
		path: "/ollama/pull",
		server: "api",
		desc: 'Start one Windows Ollama pull with bounded streamed progress. Body: {"model": string}.',
	},
	{
		method: "POST",
		path: "/ollama/pull/cancel",
		server: "api",
		desc: "Cancel Hlid's active pull request. Ollama exposes no documented global cancel operation, so partial vendor data may remain.",
	},
	{
		method: "POST",
		path: "/ollama/load",
		server: "api",
		desc: 'Create and load Hlid\'s fixed 65,536-context Ollama variant for one verified local model. Body: {"model": string}.',
	},
	{
		method: "POST",
		path: "/ollama/unload",
		server: "api",
		desc: 'Unload Hlid\'s fixed-context OpenCode variant for one Windows Ollama model. Body: {"model": string}.',
	},
	{
		method: "DELETE",
		path: "/ollama/model?model=",
		server: "api",
		desc: "Explicitly delete one Windows Ollama model after it is removed from Hlid's Ollama integration selection.",
	},
	{
		method: "POST",
		path: "/ollama/firewall",
		server: "api",
		desc: "Request Windows elevation to create Hlid's exact WSL-creator inbound TCP 11435 Hyper-V firewall rule.",
	},
	{
		method: "DELETE",
		path: "/ollama/firewall",
		server: "api",
		desc: "Request Windows elevation to remove only Hlid's exact Ollama WSL Hyper-V firewall rule.",
	},
	{
		method: "POST",

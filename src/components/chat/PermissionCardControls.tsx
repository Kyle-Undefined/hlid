...
function PermissionDecisionButtons({
	...
	const actionCount =
		...
		(message.allowSession === false ? 0 : 1) +
		...
		<div className={`grid grid-cols-2 ${actionGrid}`}>
			...
			{message.allowAlways !== false && (
				<button
					...
					className={`flex min-w-0 items-center justify-center gap-1.5 border-border px-1 py-2 text-[10px] tracking-widest text-purple-500/70 uppercase transition-colors hover:bg-purple-500/5 sm:gap-2 sm:border-l ${message.allowOnce === false && message.allowSession !== false ? "col-span-2 sm:col-span-1" : ""}`}
				>
					<Check className="h-3 w-3 shrink-0" />
					Always
				</button>

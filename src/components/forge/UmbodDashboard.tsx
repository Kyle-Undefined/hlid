...
export function UmbodDashboard({
	...
	const agents = tools?.totals?.agents ?? [];
	const projects = tools?.totals?.projects ?? [];
						...
						>
							...
							<div className="flex h-1.5 overflow-hidden bg-secondary">
								<span
									...
									style={{
										width: `${(row.decisions.allow / row.count) * 100}%`,
									}}
								/>
								<span
									className="bg-amber-500"
									style={{
										width: `${(row.decisions.approve / row.count) * 100}%`,
									}}
								/>
								<span
									className="bg-red-500"
									style={{
										width: `${(row.decisions.block / row.count) * 100}%`,
									}}
								/>
							</div>
			...
			<div className="space-y-3">
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="text-[10px] text-muted-foreground">
							Expand a call for policy context, project, session, and raw
							inputs.
						</p>
					</div>
				...
				<div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
					...
					<select
						...
						value={filters.tool}
						onChange={(event) =>
							setFilters((value) => ({ ...value, tool: event.target.value }))
						}
					...
					<select
						...
						value={filters.agent}
						onChange={(event) =>
							setFilters((value) => ({ ...value, agent: event.target.value }))
						}
						className={fieldClass}
					>
					...
					>
						...
						{["readonly", "stateful", "external", "destructive", "unknown"].map(
							(value) => (
					...
					<select
						...
						onChange={(event) =>
							setFilters((value) => ({ ...value, project: event.target.value }))
						}
						className={fieldClass}
					>
								...
								<div className="border-t border-border bg-secondary/40 p-3 space-y-3 text-xs">
									<dl className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
										{[
											...
											[
												"Matched rule",
												entry.matchedRule || "fallback / automatic",
											],
										].map(([label, value]) => (
				...
				<div className="flex items-center justify-between text-xs">
					<button
						...
						onClick={() => void loadCalls((calls?.page ?? 1) - 1)}

...
type CliProxyAccountId = (typeof OAUTH_ACCOUNTS)[number]["id"];
type RunCliProxyAction = (
	label: string,
	action: () => Promise<CliProxyStatus>,
) => Promise<void>;

function connectCliProxyAccount(id: CliProxyAccountId) {
	const actions: Record<CliProxyAccountId, () => Promise<CliProxyStatus>> = {
		codex: connectCliProxyCodexFn,
		claude: connectCliProxyClaudeFn,
		antigravity: connectCliProxyAntigravityFn,
		kimi: connectCliProxyKimiFn,
		xai: connectCliProxyXaiFn,
	};
	return actions[id]();
}

function accountStateLabel(status: CliProxyStatus, id: CliProxyAccountId) {
	if (status.accounts[id] === "connected") return "Connected";
	if (status.accounts[id] === "running") return "Waiting";
	return "Not connected";
}

function repairActionLabel(info: CliProxyStatus): string {
	if (info.versionMismatch) return `Use approved v${info.approvedVersion}`;
	if (!info.wslInstalled) return "Add WSL support";
	return "Check / repair";
}

function CliProxyOverview({
	info,
	external,
	integrationConfig,
}: {
	info: CliProxyStatus;
	external: boolean;
	integrationConfig: HlidConfig["cliproxy"];
}) {
	if (external) {
		return (
			...
			<div className="border border-border/70 bg-background/40 p-3">
				...
				<div className="flex flex-wrap gap-x-3 gap-y-1 mt-1 text-xs">
					{OAUTH_ACCOUNTS.map((account) => (
						<span key={account.id}>
							{account.label}: {accountStateLabel(info, account.id)}
						</span>
					))}
				</div>
				<p className="text-xs text-muted-foreground mt-1">
					OpenAI, Kimi, and xAI can be approved from any browser. Claude and
					Antigravity callbacks open on the Windows host. Tokens stay in Hlid's
					private integration directory.
				</p>
			</div>
			<div className="border border-border/70 bg-background/40 p-3">
				<div className="text-[10px] tracking-widest uppercase text-muted-foreground">
...
function CliProxyActions({
	...
	busy: string | null;
	external: boolean;
	onInstall: (label: "install" | "update") => void;
	onRun: RunCliProxyAction;
	onRefresh: () => void;
}) {
	...
	return (
		<div className="flex flex-wrap gap-2">
			{(!installed || external) && (
				<button
					type="button"
					disabled={Boolean(busy) || installing || info.state === "unsupported"}
				...
				>
					{busy === "install" || installing ? "Installing…" : "Install managed"}
				</button>
			)}
			{installed && !external && !running && (
				<button
					...
					onClick={() => void onRun("start", startCliProxyFn)}
					className="px-3 py-1.5 bg-primary text-primary-foreground text-[10px] tracking-widest uppercase disabled:opacity-50"
				>
					Enable
				</button>
			...
			{installed && !external && running && (
				<button
					type="button"
					disabled={Boolean(busy)}
					onClick={() => void onRun("stop", stopCliProxyFn)}
			...
			{installed &&
				...
				OAUTH_ACCOUNTS.map((account) => (
					<button
						...
						onClick={() =>
							void onRun(`oauth-${account.id}`, () =>
								connectCliProxyAccount(account.id),
							)
						}
						className="px-3 py-1.5 border border-border text-[10px] tracking-widest uppercase disabled:opacity-50"
					>
			...
			{installed && !external && (
				<>
					<button
						...
						className="px-3 py-1.5 border border-border text-[10px] tracking-widest uppercase disabled:opacity-50"
					>
						{installing ? "Installing…" : repairActionLabel(info)}
					</button>
					<ConfirmAction
						label="remove CLIProxy and saved accounts?"
						confirmText="remove"
						onConfirm={() => void onRun("remove", removeCliProxyFn)}
						trigger={(open) => (
							<button
								type="button"
								disabled={Boolean(busy)}
...
export function CliProxySection({
	...
	return (
		<section className="border border-border bg-card p-4 space-y-4">
			...
			/>
			<CliProxyActivity info={info} error={error} />
			<CliProxyActions
				info={info}
				busy={busy}
				external={external}
				onInstall={(label) => {
					...
					void run(label, installCliProxyFn);
				}}
				onRun={run}

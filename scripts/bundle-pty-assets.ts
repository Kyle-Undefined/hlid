/**
 * bundle-pty-assets.ts - run before compiling the Windows executable.
 *
 * Finds all node-pty runtime files (worker, natives, lib JS), copies them to
 * neutral generated asset names, and generates an ignored JavaScript module with
...
const generatedDir = join(repoRoot, "build", "embed-assets", "pty");
const outFile = join(generatedDir, "pty-assets.generated.js");
...
function toImportPath(repoRelPath: string): string {
	...
	return "./" + slash(relative(generatedDir, abs));
}
...
function stagedRepoRel(repoRel: string): string {
	...
	return `build/embed-assets/pty/files/${hash}-${safeBase}.asset`;
}
...
const lines: string[] = [
	...
	"// Redirected into the Windows executable by scripts/build-win.ts.",
	"",
];

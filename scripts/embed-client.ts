...
const serverFnNamesOut = resolve(
	root,
	"src",
	"server",
	"embedded-server-fn-names.ts",
);
...
const serverFnNamesContent = `${banner}
export const SERVER_FN_NAMES: Record<string, string> = {
${serverFnLines.join("\n")}
};
`;
...
writeFileSync(serverFnNamesOut, serverFnNamesContent);
console.log(
	`Embedded ${files.length} client files and ${serverFnLines.length} server function names → src/server/embedded-client.ts + embedded-server-fn-names.ts`,
);
